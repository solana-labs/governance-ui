...
import {Coefficients, DEFAULT_COEFFICIENTS, toAnchorType} from "../../QuadraticPlugin/sdk/api";
...
async function getCoefficients(coefficientsFromForm: Coefficients | undefined, existingCommunityMintPk: PublicKey | undefined, connection: Connection): Promise<Coefficients> {
...
export const addQVPlugin = async (
    ...
    const qvCoefficients = await getCoefficients(coefficientsFromForm, existingCommunityMintPk, connection);
    ...
    const qvRegistrarInstruction = await quadraticClient.program.methods
        ...
        .remainingAccounts(
            predecessorPluginProgram ? [{pubkey: predecessorPluginProgram, isWritable: false, isSigner: false}] : []
        )
        .instruction()
