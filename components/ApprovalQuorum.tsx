		...
		<div className={`${showBg ? 'bg-bkg-1' : ''}`}>
			<div className="flex items-center">
				<div className="w-full">
					...
					{progress < 100 ? (
						<p className="font-bold mb-0 text-fgd-1">{`${yesVotesRequired?.toLocaleString(undefined, {
							maximumFractionDigits: 0,
						})} ${progress > 0 ? 'more' : ''} Yes vote${yesVotesRequired > 1 ? 's' : ''} required`}</p>
					) : (
						<div className="flex items-center">
			...
			<div className="bg-bkg-4 h-2 flex flex-grow mt-2.5 rounded w-full">
				<div
					...
					className={`${progress >= 100 ? 'bg-green' : 'bg-fgd-3'} flex rounded`}
				></div>
			</div>
			{/* ) : null} */}
		</div>
