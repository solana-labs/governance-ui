...
const hasDedicatedVotingPowerUI = (plugin: PluginName): plugin is typeof pluginsWithDedicatedVotingPowerUI[number] => pluginsWithDedicatedVotingPowerUI.includes(plugin as typeof pluginsWithDedicatedVotingPowerUI[number]);

const CardForPlugin:FC<{plugin: typeof pluginsWithDedicatedVotingPowerUI[number]} & VotingCardProps> = ({plugin, role, ...props}) => {
    switch (plugin) {
        case 'NFT':
...
export const VotingPowerCards:FC<VotingCardProps> = (props) => {
    ...
    const cards = (plugins?.voterWeight ?? []).map((plugin, pluginIdx):(ReactNode | undefined) => {
        return hasDedicatedVotingPowerUI(plugin.name) ? <CardForPlugin plugin={plugin.name} {...props} key={pluginIdx} /> : undefined
    }).filter(Boolean) // filter out undefined

    const includesUnrecognizedPlugin = plugins?.voterWeight.some((plugin) => plugin.name === 'unknown')

    if (!cards.length) {
        // No dedicated plugin cards - add the vanilla card
        cards.push(<VanillaCard unrecognizedPlugin={includesUnrecognizedPlugin} {...props} />)
    }
