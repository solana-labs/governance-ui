...
import { ArrowLeftIcon, CheckCircleIcon, ExternalLinkIcon, LogoutIcon, UserCircleIcon, XCircleIcon } from '@heroicons/react/outline'
...
const MemberOverview = () => {
	...
	const [ownVoteRecords, setOwnVoteRecords] = useState<WalletTokenRecordWithProposal[]>([])

	const { walletAddress, councilVotes, communityVotes, votesCasted, hasCommunityTokenOutsideRealm, hasCouncilTokenOutsideRealm } = member!
	...
	const communityAmount = communityVotes && !communityVotes.isZero() ? useMemo(() => fmtMintAmount(mint, communityVotes), [member!.walletAddress]) : null
	const councilAmount = councilVotes && !councilVotes.isZero() ? useMemo(() => fmtMintAmount(councilMint, councilVotes), [member!.walletAddress]) : null
	...
	const getVoteRecordsAndChatMsgs = async () => {
		...
		try {
			const results = await Promise.all([getVoteRecordsByVoterMapByProposal(connection.current, selectedRealm!.programId!, new PublicKey(member!.walletAddress)), getGovernanceChatMessagesByVoter(connection!.current, GOVERNANCE_CHAT_PROGRAM_ID, new PublicKey(member!.walletAddress))])
	...
	useEffect(() => {
		...
		const handleSetVoteRecords = async () => {
			...
			const voteRecordsArray: WalletTokenRecordWithProposal[] = Object.keys(voteRecords)
				.sort((a, b) => {
					const prevProposal = proposals[a]
					const nextProposal = proposals[b]
					return prevProposal?.account.getStateTimestamp() - nextProposal?.account.getStateTimestamp()
				})
				.reverse()
				.filter((x) => proposals[x])
				.flatMap((x) => {
					const currentProposal = proposals[x]
					const currentChatsMsgPk = Object.keys(chat).filter((c) => chat[c]?.account.proposal.toBase58() === currentProposal?.pubkey.toBase58())
					const currentChatMsgs = currentChatsMsgPk.map((c) => chat[c].account.body.value)
					return {
						proposalPublicKey: x,
					...
					<a href={fmtUrlWithCluster(`/dao/${symbol}/proposal/${x.proposalPublicKey}`)} rel="noopener noreferrer" className="border border-fgd-4 default-transition hover:bg-bkg-3 css-1ug690d-StyledCardWrapepr elzt7lo0 p-4 text-xs text-th-fgd-1 mb-2 flex" key={x.proposalPublicKey}>
						<div className="w-full pr-6">
							...
							{x.chatMessages.map((msg, index) => (
								<div key={index} className={`text-xs text-fgd-3 text-xs p-2 border-t border-fgd-4`}>
									{msg}
								</div>
							))}
						</div>
						<div className="ml-auto text-fgd-3 text-xs flex flex-col">{isYesVote(x.account) ? <CheckCircleIcon className="h-4 mr-1 text-green w-4" /> : <XCircleIcon className="h-4 mr-1 text-red w-4" />}</div>
					</a>
				))}
			</div>
		</>
