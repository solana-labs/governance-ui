...
import { MultisigOptions, BespokeConfig, BespokeCouncil, BespokeInfo, RealmCreated } from './components/Steps'
import { useMemo } from 'react'
import Button from '@components/Button'
import { RealmArtifacts, RealmWizardMode, RealmWizardStep, StepDirection } from './interfaces/Realm'
import { PublicKey } from '@solana/web3.js'
import useWalletStore from 'stores/useWalletStore'
import { DEFAULT_GOVERNANCE_PROGRAM_ID, DEFAULT_TEST_GOVERNANCE_PROGRAM_ID } from '@components/instructions/tools'
...
import { getGovernanceProgramVersion, MintMaxVoteWeightSource } from '@solana/spl-governance'
...
const RealmWizard: React.FC = () => {
	...
	const [currentStep, setCurrentStep] = useState<RealmWizardStep>(RealmWizardStep.SELECT_MODE)
	const [realmAddress] = useState('')
	const [loaderMessage] = useState<LoaderMessage>(LoaderMessage.DEPLOYING_REALM)

	const [addTreasury, setAddTreasury] = useState<boolean>(false)
	const [realmId, setRealmId] = useState<any>()
	...
	const handleCreateMultisigRealm = async () => {
		...
		const programId = testRealmCheck ? DEFAULT_TEST_GOVERNANCE_PROGRAM_ID : DEFAULT_GOVERNANCE_PROGRAM_ID

		const governanceProgramId = new PublicKey(programId)
		const programVersion = await getGovernanceProgramVersion(connection.current, governanceProgramId)
	...
	const getMintMaxVoteWeight = () => {
		...
		if (form.communityMintMaxVoteWeightSource) {
			const fraction = new BigNumber(form.communityMintMaxVoteWeightSource).shiftedBy(MintMaxVoteWeightSource.SUPPLY_FRACTION_DECIMALS).toString()
	 ...
	 */
	const getTeamWallets = (): PublicKey[] | undefined => (form.teamWallets ? form.teamWallets.map((w) => new PublicKey(w)) : undefined)

	const handleCreateBespokeRealm = async () => {
		setFormErrors({})

		const { isValid, validationErrors }: formValidation = await isFormValid(CreateFormSchema, form)
	...
	const handleModeSelection = (option: RealmWizardMode) => {
		try {
			const ctl = new RealmWizardController(RealmWizardMode.ADVANCED)
			const nextStep = ctl.getNextStep(currentStep, StepDirection.NEXT)
			handleSetForm({
				governanceProgramId: process.env.DEFAULT_GOVERNANCE_PROGRAM_ID ?? DEFAULT_GOVERNANCE_PROGRAM_ID,
				yesThreshold: 100,
			})
			setController(ctl)
	...
	const isCreateButtonDisabled = () => (ctl && ctl.getMode() === RealmWizardMode.ADVANCED ? false : !form.teamWallets?.length || !form.name)
	...
	const canGoNext = (step: RealmWizardStep): boolean => {
		if (step === RealmWizardStep.BESPOKE_CONFIG) {
			...
			!form.governanceProgramId ? (errors.governanceProgramId = 'Governance Program ID is required') : null
	...
	const BoundStepComponent = useMemo(() => {
		// handleModeSelection(RealmWizardMode.ADVANCED);
		switch (currentStep) {
				...
				return (
					<BespokeConfig
						...
						onSwitch={(x: boolean) => {
							...
							handleSetForm({
								governanceProgramId: x ? DEFAULT_TEST_GOVERNANCE_PROGRAM_ID : DEFAULT_GOVERNANCE_PROGRAM_ID,
							})
						}}
					/>
				)
			...
			case RealmWizardStep.BESPOKE_INFO:
				return <BespokeInfo form={form} setForm={handleSetForm} formErrors={formErrors} />
