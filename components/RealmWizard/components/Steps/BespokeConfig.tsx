...
export const getSubtitleForMinTokenToCreate = (form: RealmArtifacts) => {
  ...
  return `Default is ${MIN_COMMUNITY_TOKENS_TO_CREATE_W_0_SUPPLY} for mints with no supply`
...
const BespokeConfig: React.FC<RealmWizardStepComponentProps> = ({
  ...
  return (
		<>
			...
			<p className="pt-8">
				Create your space to discuss + vote on proposals and certify your real world assets with Tokr.
			</p>
			<div className="pt-16">
				...
				<div className="hidden">
					<div className="pb-4 pr-10 mr-2 mt-3 hidden">
						<Input disabled label="Community Token Mint" placeholder="(Optional) Community token mint of this realm" value={form?.communityMintId} type="text" error={formErrors['communityMintId'] || formErrors['communityMint']} onChange={(evt) => setForm({ communityMintId: evt.target.value })} />
						{form?.communityMint && (
							<div className="pt-2">
						...
						<>
							<div className="pb-4 pr-10 mr-2">
								<div className="flex justify-left items-center">
									...
									<StyledLabel className="mt-1.5 ml-3">Transfer authority</StyledLabel>
								</div>
							</div>
						</>
					)}
					<div className="hidden pb-4 pr-10 mr-2 relative">
						<Input
							disabled
							...
							className="w-36"
							step="1"
							value={form.minCommunityTokensToCreateGovernance ? form.minCommunityTokensToCreateGovernance.toString() : MIN_COMMUNITY_TOKENS_TO_CREATE_W_0_SUPPLY}
							type="number"
						...
						/>
						<p className="text-gray-500 mt-1 absolute bottom-5 sm:bottom-7 left-14 ml-12 pl-12">{getMinTokensToCreatePercent(form)} of token supply</p>{' '}
					</div>

					<div className="hidden pb-4 pr-10 mr-2  mt-3 relative">
						<Input
							disabled
							...
							onChange={(evt) => {
								const value = +evt.target.value <= 1 && +evt.target.value >= 0 ? evt.target.value : 1
								setForm({
									communityMintMaxVoteWeightSource: value,
								})
							}}
						/>
						<p className="text-gray-500 mt-1 absolute bottom-5 sm:bottom-7 left-14 ml-12 pl-12">{getMintSupplyFactorPercent(form)} vote weight</p>
					</div>
					<div className="hidden pb-4 pr-10 mr-2 mt-3 ">
						<Input
				...
				{/* <div className="pb-4 pr-10 mr-2 mt-3">
					<div className="flex justify-left items-center">
						<Switch className="mt-2 mb-2" checked={isTestProgramId} onChange={(x) => onSwitch(x)} />
