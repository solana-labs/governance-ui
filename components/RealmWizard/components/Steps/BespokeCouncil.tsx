...
const BespokeCouncil: React.FC<RealmWizardStepComponentProps> = ({ setForm, form, formErrors, switchState = true, onSwitch = () => null }) => {
	...
	return (
		<>
			...
			{switchState && (
				<>
					<div className="pb-7 pr-10 w-full hidden">
						<Input disabled label="Council token mint" placeholder="(Optional) Council mint" value={form?.councilMintId} type="text" error={formErrors['councilMintId'] || formErrors['councilMint']} onChange={(evt) => setForm({ councilMintId: evt.target.value })} />
					</div>
					<div className="pb-7 pr-10 w-full pt-16 hidden" style={{ maxWidth: 512 }}>
						<ApprovalQuorumInput
							value={100}
							onChange={($e) => {
								setForm({ yesThreshold: 100 })
							}}
							onBlur={() => {
								if (!form.yesThreshold || form.yesThreshold.toString().match(/\D+/gim)) {
									setForm({
										yesThreshold: 100,
									})
								}
					...
					<TeamWalletField onInsert={handleInsertTeamWallet} onRemove={handleRemoveTeamWallet} wallets={form.teamWallets} />
				</>
			)}
		</>
	)
