...
const NewAccountForm = ({ rnft }: { rnft?: boolean | undefined }) => {
	...
	const isCurrentVersionHigherThenV1 = () => {
		return realmInfo?.programVersion && realmInfo.programVersion > PROGRAM_VERSION_V1
	}
	const { realmInfo, realm, mint: realmMint, symbol, ownVoterWeight } = useRealm()
	...
	const handleCreate = async () => {
		...
		try {
			...
			if (isValid && realmMint) {
				...
				await createTreasuryAccount(rpcContext, realm, new PublicKey(form.mintAddress), governanceConfig, tokenOwnerRecord!.pubkey, client)
				setIsLoading(false)
	...
	const schema = yup.object().shape({
		mintAddress: yup.string().test('mintAddressTest', 'Mint address validation error', async function (val: string) {
			if (val) {
				try {
					...
					const accountData = await connection.current.getAccountInfo(pubKey)
					if (!accountData) {
						return this.createError({
