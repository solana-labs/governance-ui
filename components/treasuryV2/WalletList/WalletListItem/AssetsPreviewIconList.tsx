...
export default function AssetsPreviewIconList(props: Props) {
  ...
  // Display any NFTs
  if (nfts.length) {
    previewList.push(<NFTCollectionPreviewIcon className="fill-fgd-1" />)
  ...
  if (previewList.length < 3 && otherCount > 0) {
    previewList.push(<OtherPreviewIcon className="stroke-fgd-1" />)
        ...
        >
          {previewList.map((item, i) =>
            React.cloneElement(item, {
              className: cx(item.props.className, 'w-4', 'h-4', 'rounded-sm'),
              key: i,
            })
          )}
          {remainingCount > 0 && (
