...
export async function arweaveUpload(
	...
	const estimatedManifestSize = estimateManifestSize([`${index}${imageExt}`, 'metadata.json'])
	const storageCost = await fetchAssetCostToStore([fsStat.size, manifestBuffer.length, estimatedManifestSize])
	...
	const tx = await sendTransactionWithRetryWithKeypair(anchorProgram.provider.connection, walletKeyPair, instructions, [], 'confirmed')
	...
	const imageFile = result.messages?.find((m) => m.filename === `${index}${imageExt}`)
