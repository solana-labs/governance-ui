...
import { getMintAccountLabelInfo, getSolAccountLabel, getTokenAccountLabelInfo, GovernedMultiTypeAccount } from '@utils/tokens'
import React, { useEffect } from 'react'
import { getProgramName } from '@components/instructions/programs/names'

const GovernedAccountSelect = ({ onChange, value, error, governedAccounts = [], shouldBeGoverned, governance, label, noMaxWidth }: { onChange; value; error?; governedAccounts: GovernedMultiTypeAccount[]; shouldBeGoverned?; governance?: ProgramAccount<Governance> | null | undefined; label?; noMaxWidth?: boolean }) => {
		...
		if (value) {
			...
			switch (accountType) {
				...
				case GovernanceAccountType.TokenGovernanceV2:
					return getTokenAccountLabelComponent(value.isSol ? getSolAccountLabel(value) : getTokenAccountLabelInfo(value))
	...
	function getMintAccountLabelComponent({ account, tokenName, mintAccountName, amount, imgUrl }) {
		return (
	...
	function getTokenAccountLabelComponent({ tokenAccount, tokenAccountName, tokenName, amount, imgUrl }) {
		return (
			<div className="break-all text-fgd-1 ">
				...
				<div className="space-y-0.5 text-xs text-fgd-3">
					{tokenName && (
						<div className="flex items-center">
							Token: {imgUrl && <img className="flex-shrink-0 h-4 mx-1 w-4" src={imgUrl} />}
							{tokenName}
						</div>
					)}
	...
	return (
		<Select label={label} onChange={onChange} componentLabel={getLabel(value)} placeholder="Please select..." value={value?.governance?.account.governedAccount.toBase58()} error={error} noMaxWidth={noMaxWidth}>
			{governedAccounts
				.filter((x) => (!shouldBeGoverned ? !shouldBeGoverned : x?.governance?.pubkey.toBase58() === governance?.pubkey?.toBase58()))
				.map((acc) => {
					return (
						<Select.Option key={acc.governance?.account.governedAccount.toBase58()} value={acc}>
							{getLabel(acc)}
						</Select.Option>
					)
				})}
