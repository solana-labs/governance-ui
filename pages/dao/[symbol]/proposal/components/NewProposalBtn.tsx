			...
			<Tooltip content={tooltipContent}>
				<div className={!canCreateProposal ? 'cursor-not-allowed opacity-60' : ''}>
					<Link href={fmtUrlWithCluster(`/dao/${symbol}/proposal/new${props.type ? '-' + props.type : ''}${props.string ? '?' + props.string : ''}`)}>
						<a className={`${!canCreateProposal ? 'cursor-not-allowed pointer-events-none' : 'hover:bg-bkg-3'} default-transition flex items-center ring-1 ring-fgd-3 px-3 py-2.5 text-fgd-1 text-sm focus:outline-none`}>
